...
export type CampaignRecord = {id:string;campaign_id:string;kind:'quest'|'calendar'|'clock'|'scene'|'map'|'marker'|'library_link'|'adventure';title:string;status:string;visibility:'dm'|'players'|'world';due_day?:number;linked_id?:string;data:Record<string,unknown>;created_at:string;updated_at:string}
